...
void *thread_map_pairs(void *ptr)
{
	...
	mtx.lock();
	for (int i = increment; i < word_size; i ++)
	{
...
int main()
{
	...
	struct thread_args thread1;
	struct thread_args thread2;
	struct thread_reduce_args thread3;
	struct thread_reduce_args thread4;
	// Create list of words for input file
	vector<pair<string, string>> list = read_input();
	pthread_t threads[3];
	vector<Key_Value_Pair> mapped_pairs;
	vector<Key_Value_Pair> mapped_pairs2;
	vector<Key_Value_Pair> Reduced_List;
	vector<Key_Value_Pair> Reduced_List2;
	...
	thread3.start_age = 51;
	thread3.end_age = 6543;
	pthread_create(&threads[2], NULL, thread_reduce_pairs, (void*)&thread3);
	pthread_join(threads[2], NULL);
	//Get and combine reduced KV lists
	Reduced_List2 = thread3.reduced_group;
